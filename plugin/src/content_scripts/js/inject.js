...
var isBindMark = function (el) {
    ...
    if (el.getAttribute != null &&
        ...
        el.getAttribute('class').indexOf(bindMarkText) !== -1) {
        return true;
    } else {
        return false
    }
...
function requestTagsByRepo(repoName, callback) {
    ...
    axios.get("/api/get", {params: {repo: repoName}})
        .then(rsp => {
...
function requestTagsByRepoList(repoNameArray, callback) {
    axios.get("/api/get-list", {params: {repo_list: repoNameArray}})
        .then(rsp => {
            callback(rsp.data.data)
        }).catch((error) => {
        console.error(error)
...
function saveTagsByRepo(repoName, tags) {
    axios.get("/api/save", {params: {repo: repoName, tags: tags}})
        .then(rsp => {
...
function searchTags(keyword, callback) {
    axios.get("/api/search", {params: {keyword: keyword}})
        .then(rsp => {
            callback(rsp.data.data.map(function (it) {
